...
class MongoDBManager:
    ...
    async def add_scheduled_post(self, message_data: dict, scheduled_time: datetime, user_id: int):
    ...
    async def get_post_stats(self, user_id: int) -> Dict:
    ...
    async def clear_pending_posts(self, user_id: int) -> int:
        """Clear all pending posts for a user"""
        result = await self.scheduled_posts.delete_many({
            'user_id': user_id,
            'status': 'pending'
        })
    ...
    async def get_user_channel(self, user_id: int) -> Optional[Dict]:
        """Get channel ID for a user"""
        settings = await self.bot_settings.find_one({'user_id': user_id})
        return settings
...
class TelegramSchedulerBot:
    ...
    async def start_command(self, client: Client, message: Message):
        ...
        user_settings = await self.db.get_user_channel(message.from_user.id)
        channel_status = "✅ **Configured**" if user_settings else "❌ **Not configured**"
...
Use the buttons below to get started!"""
    ...
    async def set_channel_command(self, client: Client, message: Message):
        ...
        try:
            ...
            await message.reply_text(
                ...
                f"🆔 **ID:** `{chat.id}`\n\n"
                "Now you can start forwarding posts to me!"
            )
    ...
    async def clear_command(self, client: Client, message: Message):
        ...
        deleted_count = await self.db.clear_pending_posts(message.from_user.id)
        
        if deleted_count > 0:
            self.user_last_scheduled[message.from_user.id] = datetime.now()
        
        await message.reply_text(f"🗑️ **Cleared {deleted_count} pending posts!**")
    ...
    async def show_status(self, message: Message, edit: bool = False):
        ...
        user_settings = await self.db.get_user_channel(message.from_user.id)
        stats = await self.db.get_post_stats(message.from_user.id)
        
        if not user_settings:
            status_text = "❌ **No channel configured**\n\nUse `/setchannel` to set up your channel first."
        else:
            status_text = f"📊 **Scheduling Status**\n\n"
            status_text += f"📺 **Channel:** {user_settings.get('channel_name', 'Unknown')}\n"
            status_text += f"🆔 **Channel ID:** `{user_settings['channel_id']}`\n\n"
            status_text += f"⏳ **Pending posts:** {stats['pending']}\n"
            status_text += f"✅ **Sent posts:** {stats['sent']}\n"
            
            if stats['next_post']:
                next_time = stats['next_post']['scheduled_time']
                status_text += f"⏰ **Next post:** {next_time.strftime('%Y-%m-%d %H:%M:%S')}\n"
            else:
...
• `/setchannel @channel` - Set target channel for posting
...
4. Posts will be sent to your channel automatically
...
**⚡ Features:**
• Random 1-3 hour intervals between posts
• Maintains posting order
• Persistent storage with MongoDB
• Real-time status monitoring
• Proper album handling with captions"""
    ...
    def calculate_next_schedule_time(self, user_id: int) -> datetime:
        ...
        self.user_last_scheduled[user_id] = next_time
        return next_time
    ...
    async def handle_message(self, client: Client, message: Message):
        ...
        if message.media_group_id:
            await self.handle_media_group(message, user_settings)
        else:
            # Handle single messages
            await self.schedule_single_message(message, user_settings)
    
    async def handle_media_group(self, message: Message, user_settings: dict):
        ...
        message_data = {
            ...
            'channel_id': user_settings['channel_id']
        }
        ...
        self.media_group_timers[group_id] = asyncio.create_task(
            self.process_media_group_after_delay(group_id, user_settings, message)
        )
    ...
    async def process_media_group_after_delay(self, group_id: str, user_settings: dict, sample_message: Message):
        ...
        if group_id in self.media_group_buffer:
            ...
            media_group_data = {
                'type': 'media_group',
                'items': media_items,
                'caption': album_caption,  # Store the album caption separately
                'channel_id': user_settings['channel_id']
            }
            
            # Schedule the media group as one post
            scheduled_time = self.calculate_next_schedule_time(sample_message.from_user.id)
            ...
            await sample_message.reply_text(
                ...
                f"{caption_info}\n"
                f"⏰ **Scheduled for:** {scheduled_time.strftime('%Y-%m-%d %H:%M:%S')}\n"
                f"📺 **Channel:** {user_settings.get('channel_name', 'Unknown')}\n"
                f"🆔 **Post ID:** `{post_id}`"
            )
    
    async def schedule_single_message(self, message: Message, user_settings: dict):
        ...
        scheduled_time = self.calculate_next_schedule_time(message.from_user.id)
        post_id = await self.db.add_scheduled_post(
            message_data,
            scheduled_time,
            message.from_user.id
                        ...
                        for i, item in enumerate(message_data['items']):
                            if item['type'] == 'photo':
                                media_list.append(InputMediaPhoto(
                                    media=item['content'],
                                    caption=album_caption if i == 0 else ''  # Only first item gets the album caption
                                ))
